...
aws logs put-metric-filter \
  --log-group-name /day23/devsecops/app \
  --filter-name Day23ErrorMetricFilter \
  --filter-pattern "ERROR" \
  --metric-transformations \
      metricName=Day23ErrorCount,metricNamespace=Day23DevSecOps,metricValue=1 \
  --region $AWS_REGION \
  --profile $AWS_PROFILE
```
...
aws cloudwatch describe-alarms \
  --alarm-names Day23-App-Error-Alarm \
  --region $AWS_REGION \
  --profile $AWS_PROFILE
```
...
```bash
cat > day23-dashboard.json <<EOF_DASHBOARD
{
  "widgets": [
    {
      "type": "metric",
      ...
      "properties": {
        "metrics": [
          [ "Day23DevSecOps", "Day23ErrorCount" ]
        ],
        "period": 60,
        "stat": "Sum",
        "region": "$AWS_REGION",
        "title": "Day23 Application Error Count"
      }
    },
    {
      "type": "text",
...
aws cloudwatch put-dashboard \
  --dashboard-name Day23-DevSecOps-Dashboard \
  --dashboard-body file://day23-dashboard.json \
  --region $AWS_REGION \
  --profile $AWS_PROFILE
```
...
aws logs describe-log-groups \
  --log-group-name-prefix /day23/devsecops/app \
  --region $AWS_REGION \
  --profile $AWS_PROFILE
```
